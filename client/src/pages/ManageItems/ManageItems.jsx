                        ...
                        <div className={`card rounded-4 sticky-top ${theme.card}`} style={{ top: "20px", zIndex: 1 }}>
                            <div className={`card-header pt-4 px-4 ${theme.cardHeader}`}>
                                ...
                                <p className={`small mb-0 ${theme.textMuted}`}>
                                    {editingItemId ? "Update item details below" : "Fill in the details to create a new item"}
                                </p>
                            </div>
                            <div className="card-body px-4 pb-4">
                                <form onSubmit={handleSubmit}>
                                    <div className="mb-3">
                                        <label className={`form-label fw-semibold ${theme.label}`}>Item Name</label>
                                        <input
                                            type="text"
                                    ...
                                    <div className="row mb-3">
                                        <div className="col-6">
                                            <label className={`form-label fw-semibold ${theme.label}`}>Price</label>
                                            <input
                                                type="number"
                                                step="0.01"
                                                className={`form-control ${theme.input}`}
                                        ...
                                        <div className="col-6">
                                            <label className={`form-label fw-semibold ${theme.label}`}>Quantity</label>
                                            <input
                                                type="number"
                                                className={`form-control ${theme.input}`}
                                                name="qty"
                                    ...
                                    <div className="mb-3">
                                        <label className={`form-label fw-semibold ${theme.label}`}>Category</label>
                                        <select
                                    ...
                                    <div className="d-grid gap-2">
                                        <button
                                            type="submit"
                                            className={`btn btn-lg ${editingItemId ? "btn-warning text-white" : "btn-primary"}`}
                                            disabled={loading}
                                        >
                                            {loading ? "Processing..." : editingItemId ? "Update Item" : "Create Item"}
                                        </button>
                                ...
                                <div className="table-responsive">
                                    <table className={`table align-middle mb-0 ${theme.table}`}>
                                        ...
                                        {filteredItems.length > 0 ? (
                                            filteredItems.map((item) => (
                                                <tr key={item.itemId}>
                                                    <td className="ps-4 fw-semibold">
                                                        <div>{item.name}</div>
                                                        <small className={theme.textMuted} style={{ maxWidth: "150px", display: "inline-block" }}>
                                                    ...
                                                    <td>
                                                        <span className={`badge ${item.qty > 10 ? "bg-primary" : item.qty > 0 ? "bg-warning" : "bg-danger"}`}>
                                                            {item.qty} in stock
                                                        </span>
                                                    </td>
                                            ...
                                            <tr>
                                                <td colSpan="6" className={`text-center py-5 ${theme.textMuted}`}>
                                                    No items found
                                                </td>
                                            </tr>
                                        )}
